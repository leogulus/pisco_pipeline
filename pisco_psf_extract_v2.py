...
"""
pisco_psf_extract_v2: including 3 steps
...
v2: Do all this for 4 bands using i as the coadd
...
(field,band,'sex_psf.param','psf_%s_%s.fits'%(field,band),str(seeing),str(param['satur_level_%s'%band]),str(1.1/1.7*np.pi*(seeing/0.22)**2))
print cmd
sub = subprocess.check_call(shlex.split(cmd))
...
cmd='sex final/coadd_c%s_i.fits,final/proj_coadd_c%s_%s.fits -c pisco_pipeline/config.sex -PSF_NAME %s -PARAMETERS_NAME pisco_pipeline/%s -CATALOG_NAME %s -SEEING_FWHM %s -SATUR_LEVEL %s -PHOT_APERTURES 15 -PIXEL_SCALE 0.22 -DETECT_MINAREA %s'%\
(field,field,band,'psfex_output/psf_%s_%s.psf'%(field,band),'sex_after_psf.param','a_psf_%s_%s.fits'%(field,band),str(seeing),str(param['satur_level_%s'%band]),str(1.1/1.7*np.pi*(seeing/0.22)**2))
print cmd
sub = subprocess.check_call(shlex.split(cmd))
