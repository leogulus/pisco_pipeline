...
#v1.1: add background subtraction from the annulus aroud the point source
...
def sex_band(field, band):
    ...
    cmd="sex %s -c pisco_pipeline/config_slr.sex -CATALOG_NAME %s -SEEING_FWHM %s" %\
     (os.path.join('final','coadd_c%s_%s.fits'%(field,band)),outname,str(see))
    print cmd
...
def aperature_f(field, band, aper_rad=2.2, annu_in=3., annu_out=6.):
    """
    ...
    bkg_per_px = phot_table['aperture_sum_1']/apers[1].to_pixel(w).area()
    bkg_per_star = bkg_per_px*apers[0].to_pixel(w).area()
    phot_table['aperture_sum_2']=map(cut,phot_table['aperture_sum_0']-bkg_per_star)
    phot_table['snr'] = phot_table['aperture_sum_2'] / (np.sqrt(phot_table['aperture_sum_2'])+1e-3)

    phot_table3 = phot_table
...
def create_star_fits_slr(field, g, r, i, z):
    ...
    total=total[total['aperture_sum_2_g']>0]; print 'after remove zero g', len(total)
    total=total[total['aperture_sum_2_r']>0]; print 'after remove zero r', len(total)
    total=total[total['aperture_sum_2_i']>0]; print 'after remove zero i', len(total)
    total=total[total['aperture_sum_2_z']>0]; print 'after remove zero z', len(total)

    total['magg'] = -2.5 * np.array(map(np.log10, total['aperture_sum_2_g']))
    total['magr'] = -2.5 * np.array(map(np.log10, total['aperture_sum_2_r']))
    total['magi'] = -2.5 * np.array(map(np.log10, total['aperture_sum_2_i']))
    total['magz'] = -2.5 * np.array(map(np.log10, total['aperture_sum_2_z']))
