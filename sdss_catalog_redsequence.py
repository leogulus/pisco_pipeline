## python pisco_pipeline/sdss_catalog_redsequence.py
import os
import pandas as pd
import numpy as np
...
## PARAMETER ##
core_radius=0.25
extra_name='sdss_cat2_sur0.25_fit'

df_sd=pd.read_csv('/Users/taweewat/Documents/xray_project/ned-result/total_sdss_ned.csv',index_col=0)
df_sd_cut=df_sd['name'][0:2]
# df_sd_cut=['SDSS0736+6513']
...
exception = [i.strip() for i in infile.readlines()]
total=pd.read_csv('/Users/taweewat/Documents/xray_project/ned-result/total_776_new.csv',index_col=0)
total=total[np.where(total['chips'].isin(exception),False,True)]
df_sd=total[total['SDSS']=='yes'].copy()
ra_l=[Angle(i, u.deg).to_string(unit=u.hr, sep='', precision=0, pad=True)[:4] for i in df_sd['ra'].values]
dec_l=[Angle(i, u.deg).to_string(unit=u.degree, sep='', precision=0, alwayssign=True, pad=True)[:5] for i in df_sd['dec'].values]
df_sd['name']=['SDSS'+i+j for i, j in zip(ra_l, dec_l)]

df_sd_cut=df_sd['name'][:]
print "total:%i"%len(df_sd_cut)

for i, field in enumerate(df_sd_cut):
    if field in exception:
        continue    
    print "%i/%i: %s"%(i, len(df_sd_cut), field)
    df0=pd.read_csv('/Users/taweewat/Documents/red_sequence/sdss_table/b5_{}.csv'.format(field),index_col=0)

    # base=pd.read_csv('/Users/taweewat/Documents/xray_project/ned-result/total_sdss_ned.csv',index_col=0)
    RA = df_sd[df_sd.name == field].ra.values[0]
    DEC = df_sd[df_sd.name == field].dec.values[0]
    redshift = df_sd[df_sd.name == field].redshift.values[0]
    sepi = df_sd[df_sd.name == field].sep.values[0]
    typei = df_sd[df_sd.name == field].type.values[0]

    df0['MAG_g']=df0['modelMag_g']
    df0['MAG_r']=df0['modelMag_r']
    df0['MAG_i']=df0['modelMag_i']
    df0['MAGERR_g']=df0['modelMagErr_g']
    df0['MAGERR_r']=df0['modelMagErr_r']
    df0['MAGERR_i']=df0['modelMagErr_i']

    c5 = SkyCoord(ra=df0['ra'].values*u.degree, dec=df0['dec'].values*u.degree)
    ...
    dfi['weight']=dfi.apply(ex.size_f, args=(core_radius,), axis=1)
    z_gmr=dfi['z_gmr'].dropna()
    z_rmi=dfi['z_rmi'].dropna()
    w_gmr=dfi.dropna(subset=['z_gmr'])['weight']
    w_rmi=dfi.dropna(subset=['z_rmi'])['weight']
    ...
    print lum_factor, density_factor, signal
    ...
    plt.subplot(1,4,1)
    dir='/Users/taweewat/Documents/xray_project/code_github/downloads/'
    plt.imshow(image.imread(dir+"%ssmall.jpg"%field))
    ...
    plt.axvline(redshift,color='red',label='z:%.2f,%.1f",%s'%(redshift,sepi,typei))
    ...
    plt.suptitle(field)
    plt.tight_layout()
    plt.savefig('/Users/taweewat/Documents/red_sequence/panstarr_color_plots/%s_%.3f_%s.png' %(extra_name,signal,field), dpi=120)
    plt.close()
    
    dfi.to_csv("/Users/taweewat/Documents/pisco_code/slr_output/galaxy_%s_final_%s.csv"%(extra_name,field))
